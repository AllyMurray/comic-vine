  ...
  describe('serialization utilities', () => {
    ...
    describe('deserializeValue', () => {
      ...
      it('should deserialize object values', () => {
        const result = deserializeValue<{ id: number; name: string }>('{"id":1,"name":"test"}');
        expect(result).toEqual({ id: 1, name: 'test' });
      });

      it('should deserialize array values', () => {
        const result = deserializeValue<Array<string | number>>('[1,2,"three"]');
      ...
      it('should throw error for invalid JSON', () => {
        expect(() => deserializeValue('invalid json')).toThrow(DynamoDBStoreError);
      });
    });
  });
  ...
  describe('timing utilities', () => {
    ...
    describe('calculateBackoffDelay', () => {
      ...
      it('should include random jitter', () => {
        const delays = Array(10).fill(null).map(() => calculateBackoffDelay(2, 100));
  ...
  describe('performance utilities', () => {
    ...
    describe('measureExecutionTime', () => {
      ...
      it('should handle operation errors', async () => {
        ...
        await expect(measureExecutionTime(operation)).rejects.toThrow('Test error');
      });
    });
  ...
  describe('retry utilities', () => {
    describe('retryWithBackoff', () => {
      ...
      it('should retry on throttling errors', async () => {
        ...
        const result = await retryWithBackoff(operation, mockConfig, 'throttle-op');
      ...
      it('should not retry on non-throttling errors', async () => {
        ...
        await expect(retryWithBackoff(operation, mockConfig, 'validation-op'))
      ...
      it('should throw ThrottlingError after max retries', async () => {
        ...
        await expect(retryWithBackoff(operation, mockConfig, 'max-retry-op'))
      ...
      it('should throw DynamoDBStoreError for other persistent failures', async () => {
        ...
        (persistentError as any).name = 'ThrottlingException'; // Make it retryable
        
        const operation = vi.fn().mockRejectedValue(persistentError);
        
        // Override to make it not a throttling error for final throw
        vi.spyOn(require('./utils.js'), 'isThrottlingError').mockReturnValue(false);
        
        await expect(retryWithBackoff(operation, mockConfig, 'persistent-op'))
      ...
      it('should work with circuit breaker', async () => {
        ...
        const result = await retryWithBackoff(operation, mockConfig, 'cb-op', circuitBreaker);
