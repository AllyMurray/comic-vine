...
// Mock crypto.randomUUID to generate unique UUIDs
let uuidCounter = 0;
vi.mock('node:crypto', () => ({
...
describe('DynamoDBRateLimitStore', () => {
  ...
  describe('basic rate limiting', () => {
    ...
    it('should block requests over limit', async () => {
      ...
      // Check current status before the blocking check
      const status = await store.getStatus(resource);
      console.log(
        'Before canProceed check - remaining:',
        status.remaining,
        'limit:',
        status.limit,
      );
  ...
  describe('error handling', () => {
    ...
    it('should throw error when store is destroyed', async () => {
      ...
      await expect(store.canProceed('test')).rejects.toThrow(
        'DynamoDBRateLimitStore has been destroyed',
      );
    });
