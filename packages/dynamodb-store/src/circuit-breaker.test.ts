...
describe('CircuitBreaker', () => {
  ...
  let mockConfig: DynamoDBStoreConfig;
  ...
  describe('failure handling', () => {
    it('should count severe errors towards failure threshold', async () => {
      ...
      (severeError as Error & { name: string }).name = 'ServiceUnavailable';
      const mockOperation = vi.fn().mockRejectedValue(severeError);

      // First failure
      await expect(
        circuitBreaker.execute(mockOperation, 'test-op'),
      ).rejects.toThrow();
    ...
    it('should count throttling errors towards failure threshold', async () => {
      ...
      (throttlingError as Error & { name: string }).name =
        'ThrottlingException';
    ...
    it('should reset failure count on successful operations', async () => {
      ...
      // Cause a failure
      await expect(
        circuitBreaker.execute(failingOperation, 'fail-op'),
      ).rejects.toThrow();
      expect(circuitBreaker.getStatus().failureCount).toBe(1);
  ...
  describe('open state behavior', () => {
    beforeEach(async () => {
      ...
      for (let i = 0; i < 3; i++) {
        await expect(
          circuitBreaker.execute(mockOperation, 'trip-op'),
        ).rejects.toThrow();
      }
    });
  ...
  describe('timeout handling', () => {
    it('should timeout long-running operations', async () => {
      ...
      await expect(
        circuitBreaker.execute(slowOperation, 'slow-op'),
  ...
  describe('disabled circuit breaker', () => {
    ...
    it('should bypass circuit breaker when disabled', async () => {
      ...
      for (let i = 0; i < 5; i++) {
        await expect(
          circuitBreaker.execute(mockOperation, 'bypass-op'),
        ).rejects.toThrow();
      }
    ...
    it('should still enforce timeouts when disabled', async () => {
      ...
      await expect(
        circuitBreaker.execute(slowOperation, 'timeout-bypass'),
      ).rejects.toThrow(OperationTimeoutError);
    });
  });
  ...
  describe('configuration options', () => {
    it('should use custom failure threshold', async () => {
      ...
      await expect(
        customBreaker.execute(mockOperation, 'single-fail'),
      ).rejects.toThrow();
    ...
    it('should use custom recovery timeout', async () => {
      ...
      const mockOperation = vi.fn().mockRejectedValue(severeError);
      await expect(
        customBreaker.execute(mockOperation, 'trip'),
      ).rejects.toThrow();
    ...
    it('should use custom operation timeout', async () => {
      ...
      await expect(
        customBreaker.execute(slowOperation, 'custom-timeout'),
      ).rejects.toThrow(OperationTimeoutError);
    });
  });
  ...
  describe('edge cases', () => {
    it('should handle operations that throw non-Error objects', async () => {
      ...
      await expect(
        circuitBreaker.execute(mockOperation, 'string-error'),
    ...
    it('should handle operations that throw null/undefined', async () => {
      ...
      await expect(
        circuitBreaker.execute(mockOperation, 'null-error'),
      ).rejects.toBeNull();
    ...
    it('should handle rapid state transitions', async () => {
      ...
      for (let i = 0; i < 3; i++) {
        await expect(
          circuitBreaker.execute(failOperation, 'rapid-fail'),
        ).rejects.toThrow();
      }
      expect(circuitBreaker.getStatus().state).toBe(CircuitBreakerState.OPEN);
      ...
      expect(circuitBreaker.getStatus().state).toBe(
