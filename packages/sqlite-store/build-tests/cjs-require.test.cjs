...
function testCJSRequire() {
  console.log('Testing sqlite-store CJS require...');
  ...
  try {
    ...
    console.log('✅ sqlite-store CJS require test passed');
    return true;
  } catch (error) {
    console.error('❌ sqlite-store CJS require test failed:', error.message);
    return false;
  }
}
